  ...
  void initState() {
    ...
    loadPuzzle().then((value) {
      setState(() {
        puzzle = value;
        userGrid =
            List.generate(puzzle.puzzleHeight, (index) => List.generate(puzzle.puzzleWidth, (index) => 0));
      });
    });
  }
      ...
      itemBuilder: (BuildContext context, int index) {
        return GestureDetector(
          onTap: () {
            ...
            for (var i = 0; i < puzzle.puzzleHeight; i++) {
              if (listEquals(userGrid[i], puzzle.solution[i])) {
                counter++;
              }
            }
            if (counter == puzzle.puzzleHeight) {
